...
def test_daqmx_linear_voltage():
    ...
    t0 = tf.group_0.cDAQ1Mod2_ai0.wf_start_time
    assert t0 == datetime(2019, 10, 2, 21, 31, 10, 787163).astimezone(timezone.utc)
    dt = tf.group_0.cDAQ1Mod2_ai0.wf_increment
    assert dt == 0.001
...
def test_daqmx_polynomial_voltage():
    ...
    t0 = tf.group_0.PXI1Slot7_ai0.wf_start_time
    assert t0 == datetime(2019, 10, 2, 21, 44, 38, 544740).astimezone(timezone.utc)
    dt = tf.group_0.PXI1Slot7_ai0.wf_increment
    assert dt == 0.001
...
def test_daqmx_digital_1ch2lines():
    ...
    assert group.PXI1Slot7_port0_line0.data == [0] * 10
    assert group.PXI1Slot7_port0_line1.data == [1] * 10
